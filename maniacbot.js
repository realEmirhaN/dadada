...
var commands = {
	...
	"atla": {
		"usage": "<index>",
		"description": "Sıradaki dizine göre belirli bir şarkıya atlar.  ",
		"process": function(message, args){
			if(message.member.voiceChannel !== undefined){
				if(songQueue.length > 0){
					...
					if(Number.isInteger(index)){
						...
						botChannel.send(`\`${args[0]}\` geçersiz endeks`, {reply: message});
					}
				} else{
					botChannel.send("Şarkı yok :sob:", {reply: message});
				}
			} else{
				botChannel.send("Müziğimi duymak için, bir ses kanalında olmalısın :cry:", {reply: message});
			}
		}
	},
	"rastgele": {
		"usage": "",
		"description": "Kuyruktaki şarkıları rastgele seçer.",
		"process": function(message, args){
			if(message.member.voiceChannel !== undefined){
				if(songQueue.length > 0){
					...
					botChannel.send("Başka şarkı yok :sob:", {reply: message});
				}
			} else{
				botChannel.send("Müziğimi duymak için, bir ses kanalında olmalısın :cry:", {reply: message});
			}
	...
	"sil": {
		"usage": "<index>",
		"description": "Sıradaki şarkı kuyruğunu veya belirli bir şarkıyı temizler",
		"process": function(message, args){
			if(message.member.voiceChannel !== undefined){
				if(songQueue.length === 0){
					botChannel.send("Temizleyecek şarkı yok", {reply: message});
				} else if(args.length > 0){
					var index = Number.parseInt(args[0]);
					if(Number.isInteger(index)){
						botChannel.send(`\`${songQueue[index - 1].title}\` şarkı kuyruğundan silindi`, {reply: message});
						songQueue.splice(index - 1, 1);
						if(index - 1 <= currentSongIndex){
							currentSongIndex--;
						}
					} else{
						botChannel.send(`\`${args[0]}\` geçersiz bir endeks`, {reply: message});
					}
				} else{
	...
	"kuyruk": {
		"usage": "",
		"description": "Şarkının kuyruğunu karıştırır .",
		"process": function(message, args){
			if(message.member.voiceChannel !== undefined){
				if(shuffle){
					shuffle = false;
					botChannel.send("Kuyruk karıştırma de-aktif", {reply: message});
				} else{
					shuffle = true;
					botChannel.send("Kuyruk karıştırma aktif", {reply: message});
				}
			} else{
				botChannel.send("You can't hear my music if you're not in a voice channel :cry:", {reply: message});
			}
	...
	"otodeğiş": {
		"usage": "",
		"description": Şarkı sırasının şarkılarını otomatik olarak değiştirir",
		"process": function(message, args){
			if(message.member.voiceChannel !== undefined){
				if(autoremove){
					autoremove = false;
					botChannel.send("Otomatik müzik değişme de-aktif", {reply: message});
				} else{
					autoremove = true;
					botChannel.send("Otomatik müzik değişme aktif", {reply: message});
				}
			} else{
				botChannel.send("Ses kanalında değilseniz, müziğimi duyamazsınız :cry:", {reply: message});
			}
		}
	},
	"şarkı": {
		"usage": "",
		"description": "Şu anda çalan şarkı hakkında bilgi verir",
		"process": function(message, args){
			if(songQueue.length > 0){
				botChannel.send(`Mevcut şarkı \`${songQueue[currentSongIndex].title}\` :musical_note:, added by ${songQueue[currentSongIndex].user}`, {reply: message});
			} else{
				botChannel.send("Kuyrukta hiç şarkı yok", {reply: message});
			}
		}
	},
	"müzik": {
		"usage": "",
		"description": "Su anda kuyrukta bulunan sarkilarin bir listesini verir.",
		"process": function(message, args){
			if(songQueue.length > 0){
				var songList = "";
				...
				botChannel.send("Şu anda şarkı kuyruğu var:\n" + songList, {reply: message});
			} else{
				botChannel.send("Kuyrukta hiç şarkı yok", {reply: message});
			}
		}
